...
def forest():
    ...
    return pd.DataFrame([[30,100,200,300,2,10],[10,99,200,300,3,-1],
                         [80,2,4,6,1,70],[70,2,3,6,1,-1],[90,9,9,9,0,-1]],
                        columns=['rowId','x','y','z','radius','link'])


def test_every_source_node_and_edge_retained_without_bridges():
    df = forest(); origin = [381728,313344,558592]
    ...
    assert np.max(np.abs(restored-df[['x','y','z']].values*8)) < .04
    assert np.isclose(nodes[0,4],.008) and np.isclose(nodes[2,4],.008)
...
def test_invalid_source_is_rejected_instead_of_repaired(fault):
    df=forest()
    if fault=='cycle': df.loc[1,'link']=30
    if fault=='dangling': df.loc[0,'link']=12345
    if fault=='duplicate': df.loc[0,'rowId']=10
    if fault=='nonfinite': df.loc[0,'x']=float('nan')
    with pytest.raises(ValueError): encode_forest(df,[0,0,0],[8,8,8])


def test_no_assumed_voxel_scale():
    with pytest.raises(ValueError): encode_forest(forest(),[0,0,0],[8,8,9])
...
def test_real_bundle_activity_references_only_real_nodes_and_edges():
    ...
    edge=circuit.doc['edges']
    real_weights={(s,t):w for s,t,w in zip(edge['source'],edge['target'],edge['weight'])}
    assert result.activations and result.pulses
    assert all(a.body_id in circuit.node_meta for a in result.activations)
    assert all(real_weights[(p.source,p.target)]==p.weight for p in result.pulses)
