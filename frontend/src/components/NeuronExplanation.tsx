...
export function NeuronExplanation({node,sim}:{node:CircuitNode;sim:SimulationEnvelope|null}) {
  ...
  return <div className="neuron-explanation">
    ...
    <p className="neuron-run-role">{info.role}</p><details><summary>Evidence for this explanation</summary><small>{info.basis}</small>{info.source && <a href={info.source} target="_blank" rel="noreferrer">Read the research ↗</a>}</details>
  </div>
}
