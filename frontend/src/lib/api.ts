...
export const api = {
  simulateSequence: (events:string[], interpretation_tickets:(string|null)[] = events.map(()=>null)) => json<SimulationEnvelope>('/simulate-sequence', {method:'POST',body:JSON.stringify({events, interpretation_tickets})}),
  ...
  simulate: (text: string, lesion: number[] = [], interpretation_ticket:string|null = null) =>
    json<SimulationEnvelope>('/simulate', {
      method: 'POST',
      body: JSON.stringify({ text, lesion, interpretation_ticket }),
    }),
