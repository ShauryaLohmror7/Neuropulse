...
export default function App() {
  ...
  const runSimulation = useCallback(async (text: string) => {
    ...
    try {
      ...
      const envelope = continuity ? await api.simulateSequence(nextEvents,nextTickets) : await api.simulate(text,[],receipt.ticket)
      ...
      <main className="workspace">
        <aside className={`experience-rail ${phase !== 'idle' ? 'has-run' : ''}`}>
          ...
          {phase === 'idle' ? <ExperienceInput onSimulate={runSimulation} disabled={!data} /> : <div className="run-story">
            ...
            {envelope && <p className="interpreter-attribution">{envelope.experience.parser.startsWith('gemini:')?'AI scene interpretation · approximate cues':'Local scene interpretation'} · outcome calculated from the network</p>}
